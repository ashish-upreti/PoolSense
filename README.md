...
- Saved Pool Recommendations workspace for `NewRecommendation` reports generated by background polling, with filters, report links, and saved-report troubleshooting.
...
- `NyraRetrieval`
  - `BaseUrl` — NYRA KnowledgeBase retrieval service endpoint used by KB document retrieval checks
...
7. When the report is ready, ask follow-up troubleshooting questions. Answers are generated from the saved recommendation, reasoning, failure pattern, and similar incidents for that pool.
...
8. New-pool recommendation results are stored in `dbo.processed_source_events` as `NewRecommendation` records and surfaced in the Pool Recommendations UI.
9. Project configuration and ingestion status drive application scoping and operational behavior.
